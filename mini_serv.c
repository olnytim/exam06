...
typedef struct s_client {
    int id;
    char msg[1024];
    int msg_len;  // Длина сообщения, которое накапливается
} t_client;
    ...
    while (1) {
        ...
        for (int fd = 0; fd <= max; fd++) {
            if (FD_ISSET(fd, &readset) && fd == serv) {
                ...
                max = max > cur ? max : cur;
                clients[cur].id = next++;
                clients[cur].msg_len = 0;  // Инициализируем длину сообщения
            ...
            if (FD_ISSET(fd, &readset) && fd != serv) {
                int res = recv(fd, toread, sizeof(toread) - 1, 0);
                if (res <= 0) {
                    ...
                    toread[res] = '\0';  // Заканчиваем строку в буфере
                    for (int i = 0; i < res; ++i) {
                        clients[fd].msg[clients[fd].msg_len++] = toread[i];
                        if (clients[fd].msg[clients[fd].msg_len - 1] == '\n') {
                            clients[fd].msg[clients[fd].msg_len - 1] = '\0';  // Завершаем строку
                            sprintf(towrite, "client %d: %s\n", clients[fd].id, clients[fd].msg);
                            sendall(fd);
                            clients[fd].msg_len = 0;  // Очищаем накопитель после отправки
                        }
                    }
                }
            }
